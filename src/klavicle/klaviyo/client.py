...
class KlaviyoClient:
    ...
    # List Operations
    async def get_lists(self, page_cursor: Optional[str] = None) -> Any:
        """Get all lists with optional pagination."""
        return self.client.Lists.get_lists(page_cursor=page_cursor)
    ...
    async def add_profiles_to_list(self, list_id: str, profile_ids: list[str]) -> Any:
        ...
        return self.client.Lists.create_list_relationships_profiles(
            list_id, cast(Any, data)
        )
    ...
    async def remove_profiles_from_list(
        ...
        return self.client.Lists.delete_list_relationships_profiles(
            list_id, cast(Any, data)
        )

    # Segment Operations
    async def get_segments(self, page_cursor: Optional[str] = None) -> Any:
        """Get all segments with optional pagination."""
        return self.client.Segments.get_segments(page_cursor=page_cursor)
    ...
    async def create_segment(self, name: str, definition: dict) -> Any:
        ...
        return self.client.Segments.create_segment(cast(Any, data))
    ...
    async def add_tags(
        ...
        if resource_type == "list":
            return self.client.Tags.tag_lists(resource_id, cast(Any, tag_data))
        elif resource_type == "segment":
            return self.client.Tags.tag_segments(resource_id, cast(Any, tag_data))
        else:
    ...
    async def remove_tags(
        ...
        if resource_type == "list":
            return self.client.Tags.remove_tag_from_lists(
                resource_id, cast(Any, tag_data)
            )
        elif resource_type == "segment":
            return self.client.Tags.remove_tag_from_segments(
                resource_id, cast(Any, tag_data)
            )
        else:
            raise ValueError(f"Unsupported resource type: {resource_type}")

    async def get_tags(self, page_cursor: Optional[str] = None) -> Any:
        """Get all tags with optional pagination."""
        return self.client.Tags.get_tags(page_cursor=page_cursor)
